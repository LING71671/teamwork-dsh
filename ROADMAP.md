...
| Serial integration and final verification | Read-only three-way planner and host/DSH preview are wired. Internal integration executor now has durable source leases, exclusive file publication, original backups, merged snapshot and independent final command acceptance, tested against real temporary projects. Still needs operator opt-in configuration, host commands/status/events, scheduler/cancel wiring, artifact registration and conflict-resolution work items. The plugin does not dispatch this executor yet. |
| Integration and unknown-process recovery | Internal journal reconciles file intents across real child-process exits and SQLite reopen; unknown acceptance dispatch remains blocked rather than repeated. Still needs user-facing evidence-backed operator resolution, lifecycle recovery dispatch and unknown DSH process reconciliation; never infer exit from a reused PID. |
