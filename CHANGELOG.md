...
- Explicit persistent CLI service plus default attached IPC-owner lifecycle. Real DSH tests prove autonomous writeback after launcher exit and resumable pause after attached-owner death. Operator-only generation-bound status/stop, atomic whole-runtime pause receipts, drain/interrupt escalation, retained budget/holds after restart and unknown-exit ownership retention are implemented. Startup observation timeout never authorizes replacement; record publication failures pause recovered work before release. Windows record replacement retries the same metadata operation only. No startup registration, automatic crash restart or multi-day soak claim.
...
- 243 tests locally, including actual DSH revision → reference tools → independent per-requirement review → explicit integration, plus transitive invalidation, stale credentials/Gate, narrowed scope, rollback, history-aware inspection, queued SQLite recovery, root/subtree workflow controls and attached/persistent service lifecycle. Not part of v0.2.0-dev.1.
