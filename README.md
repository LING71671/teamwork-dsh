...
`v0.2.0-dev.1` 有 35 项测试，当前 0.3 工作树有 207 项。`npm test` 会构建源码并执行测试，包括真实 DSH SDK/Cordis 的离线模型适配器、逐项需求评审与变更范围、需求修订与旧证据失效、三方冲突解决后独立验证、显式集成到临时项目、保留用户改动、最终验收，以及进程退出恢复；不需要 API key，也不验证真实模型的任务效果。
